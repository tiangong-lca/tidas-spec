...
lastReviewedNote: "Reviewed for tidas-spec #31 / PR #32: preserve established mix, technology-specific and at/to naming while adding scoped evidence-based clarification; ownership, schema and consumer gates unchanged."
related:
  - README.md
  - .docpact/config.yaml
  - LICENSE
...
- The only publication automation is the manually invoked `.github/workflows/publish-spec-release.yml` workflow. It defaults to `blocked`; `preflight` checks the exact candidate, OIDC, and SDK credential without release writes, and only explicit `publish` approval can write to npm, GitHub Release, or SDK. It verifies the qualified source commit plus archive and manifest digests, and publishes or verifies the same exact archive in both channels. An exact replay may fill a missing channel; any existing version with conflicting bytes fails closed. npm publication uses GitHub OIDC Trusted Publishing, not a static npm token; OIDC availability and the external SDK-dispatch credential are checked before publication writes. The npm package and its matching Trusted Publisher must already exist before this workflow can make its first npm publication.
- Only after both immutable channels match does the workflow emit one `tidas_spec_released` repository-dispatch event. Its `event_key` is derived from package, version, archive digest, and manifest digest; consumers must treat that key and the exact source/archive/manifest fields as the release identity and reject stale or conflicting replays.
...
- A candidate may be built and verified before publication, and it may be used for continued validation. Formal publication requires the reviewed provenance record, exact qualified release identity, credentials, and guarded channel checks. The 2026-09-19 owner declarations cover the nine embedded classification groups and authorize public redistribution of the whole shipped package, including ILCD-derived structural conventions; they do not assert ILCD authorship or independent third-party license proof. The 52 Draft 7 `$ref` assertion-sibling locations remain unchanged as an explicitly accepted first-release cross-validator caveat, not a claim of evaluator equivalence.

**Licensing**

- `LICENSE` reproduces the MIT notice of the tools source that supplies the first extracted assets.
- That notice alone does not establish the license of every imported artifact. Preserve the separate source-byte evidence and rights-owner declarations; never describe the whole package as MIT-licensed on the strength of the source notice alone.
- [`docs/provenance.md`](docs/provenance.md) is the standing record: verified source facts, the named owner declaration for nine embedded classification groups, the later whole-package public-redistribution authorization, its limits, and the accepted Draft 7 caveat. Changing a rights or semantic disposition is a reviewed content decision with named authority, not an edit that makes a blocker disappear.
