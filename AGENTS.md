...
- The only publication automation is the manually invoked `.github/workflows/publish-spec-release.yml` workflow. It defaults to `blocked`, requires an explicit `publish` approval, verifies the qualified source commit plus archive and manifest digests, and publishes or verifies the same exact archive in npm and GitHub Release. An exact replay may fill a missing channel; any existing version with conflicting bytes fails closed. npm publication uses GitHub OIDC Trusted Publishing, not a static npm token; OIDC availability and the external SDK-dispatch credential are checked before publication writes. The npm package and its matching Trusted Publisher must already exist before this workflow can make its first npm publication.
